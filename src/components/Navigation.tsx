  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-blue-100/70 backdrop-blur-md shadow-md border-b border-blue-300">
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        ...
        {/* Mobile Nav */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-blue-100/95 rounded-lg shadow-lg py-4 px-4 mt-2 space-y-2">
            {navItems.map((item) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="block text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors duration-200"
              >
                {item}
              </a>
            ))}
